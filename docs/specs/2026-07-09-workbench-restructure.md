...
> **Revision note (2026-07-11):** two changes after the template review. (1) The `lab/` tier is
> **dropped** — `design-system-lab/` is stale (v1.0.0 vs canonical 1.3.0) and the DS `gallery/` +
> `sandbox/` already cover the dev/preview role; it moves to `archive/design-system-lab/` instead.
> §5, §8, §10, §11 updated accordingly. (2) §9 gains a **consumer-formatter rule**: consuming
> projects must exclude `design-system/` from their formatter (Biome `"!design-system"` in
> `files.includes`), otherwise format hooks rewrite the copied files and the extract tool's
> overwrite protection halts on false conflicts forever (already observed: web-vite's bundled
> `theme-toggle.js` had drifted by formatting alone).
...
reorganise its folders into `libraries / scaffolds / tools / docs`, add a unified dashboard
...
│                                    plus design-system-lab — retired 2026-07-11, was stale at v1.0.0)
...
Support tiers: `tools` (the extract script), `docs`, `reference`, `archive`. The library
dev/preview surface is the design-system's own `gallery/` and `sandbox/` — no separate lab tier
(2026-07-11 revision).
...
- **Content:** three tier sections as card grids —
  ...
  - **Docs** → link to `docs/`. (No lab card — 2026-07-11 revision.)
...
**Consumer-formatter rule (2026-07-11):** every consuming project excludes its copied
`design-system/` from its formatter — for Biome, `"!design-system"` in `files.includes`. The copy
is read-only in consumers; letting a format hook rewrite it makes every file "locally modified"
and turns the overwrite protection into a permanent false alarm. The web-vite scaffold ships this
exclusion so new projects inherit it.
...
2. **Move with history** (`git mv`):
   ...
   - `design-system-lab/` → `archive/design-system-lab/` (2026-07-11 revision — stale, retired)
...
3. **Re-point references.** Enumerate every occurrence with grep over these targets:
   ...
   - (`archive/design-system-lab/` is dead storage — its internal links are not re-pointed.)
...
- [ ] No broken relative links in `docs/` or the root dashboard (link-check).
